...
export default function HomePage() {
  ...
  const [form, setForm] = useState({
    ...
    address: '', expected_gmv: '', channel_gmv: '',
  });
        ...
        <div style={S.body}>
          <div style={{ textAlign: 'center', padding: '60px 20px', animation: 'fadeUp .5s ease' }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button style={S.btnBack} onClick={() => { setView('apply'); setApplyStep(1); setForm({ name:'',email:'',link:'',followers:'',niche:'',avg_views:'',avg_viewers:'',platform:'',ct:'',address:'',expected_gmv:'',channel_gmv:'' }); setErr(''); }}>Đăng ký lần khác</button>
            </div>
          </div>
        </div>
      </div>
