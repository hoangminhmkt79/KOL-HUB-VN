...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    const { name, email, link, followers, niche, avg_views, avg_viewers, platform, ct, address, score, potential, expected_gmv } = req.body;
    ...
    try {
      const r = await pool.query(
        `INSERT INTO creators (name,email,tiktok_link,followers,niche,avg_views,avg_viewers,platform,content_type,address,score,potential,expected_gmv)
        ...
        [name, email, link, parseInt(followers)||0, niche, parseInt(avg_views)||0, parseInt(avg_viewers)||0,
         platform, ct, address||'', parseFloat(score)||0, potential||'low', expected_gmv||'']
      );
