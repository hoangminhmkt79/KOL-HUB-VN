...
export default function HomePage() {
  ...
  const [form, setForm] = useState({
    ...
    avg_views: '', avg_viewers: '', platform: '', ct: '', address: '',
  });
  ...
  const doStep1 = () => {
    if (!form.name || !form.email || !form.link || !form.followers || !form.avg_views) {
  ...
  const doSubmit = async () => {
    if (!form.address) { setErr('Điền địa chỉ nhận sample.'); return; }
    setLoading(true);
    try {
        ...
        <div style={S.body}>
          <div style={{ textAlign: 'center', padding: '60px 20px', animation: 'fadeUp .5s ease' }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button style={S.btnBack} onClick={() => { setView('apply'); setApplyStep(1); setForm({ name:'',email:'',link:'',followers:'',niche:'',avg_views:'',avg_viewers:'',platform:'',ct:'',address:'' }); setErr(''); }}>Đăng ký lần khác</button>
              <button style={S.btnMain} onClick={() => window.location.href='/admin'}>Vào Admin →</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={S.page}>
        <div style={S.body}>
          ...
          {applyStep === 3 && (
            <>
              <div style={{ marginBottom: 8 }}>
                <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: '-.5px', marginBottom: 4 }}>Cuối cùng — địa chỉ nhận sample</div>
                <div style={{ fontSize: 13, color: '#71717a' }}>Giao tận nhà sau khi đơn được duyệt</div>
              </div>
              <div style={S.card}>
                <label style={S.lbl}>Địa chỉ đầy đủ *</label>
                <textarea style={{ ...S.input, resize: 'none', lineHeight: 1.6, marginTop: 7 }} rows={3} value={form.address} onChange={set('address')} placeholder="Số nhà, tên đường, phường/xã, quận/huyện, tỉnh/thành phố" />
              </div>
        ...
        <div style={{ maxWidth: 640, margin: '0 auto', padding: '28px 20px' }}>
          ...
          <div style={{ textAlign: 'center', background: '#fff', border: '1px solid #f0f0f0', borderRadius: 20, padding: '28px 24px' }}>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
              ...
              <a href="/admin" style={{ ...S.btnGhost, display: 'inline-flex', alignItems: 'center', padding: '14px 22px', fontSize: 14 }}>Admin →</a>
            </div>
          </div>
        </div>
      </div>
