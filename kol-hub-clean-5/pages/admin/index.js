...
export default function AdminLogin() {
  ...
  const login = async (e) => {
    ...
    try {
      ...
      if (d.ok) {
        // Lưu token — không lưu mật khẩu
        sessionStorage.setItem('kol_token', d.token);
  ...
  const S = {
    ...
    card: { background: '#fff', border: '1px solid #f0f0f0', borderRadius: 24, padding: '48px 40px', width: '100%', maxWidth: 360, textAlign: 'center', boxShadow: '0 4px 32px rgba(0,0,0,.06)' },
    ...
    input: { background: '#fafafa', border: '1.5px solid #e4e4e7', borderRadius: 12, padding: '14px 16px', width: '100%', outline: 'none', textAlign: 'center', letterSpacing: '.2em', fontSize: 20, color: '#0f0f1a', fontFamily: 'inherit' },
    btn: { width: '100%', background: loading ? '#a78bfa' : '#7c3aed', color: '#fff', border: 'none', borderRadius: 12, padding: 14, fontWeight: 700, fontSize: 15, cursor: loading ? 'not-allowed' : 'pointer', marginTop: 4, fontFamily: 'inherit', transition: 'background .2s' },
    ...
    <>
      ...
      <div style={S.page}>
        <div style={S.card}>
          ...
          {/* Không có link về trang chủ để tránh lộ route admin */}
        </div>
      </div>
    </>
  );
