          ...
          {tab === 'apps' && (
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button style={{ ...BTN_G, fontSize: 12 }} onClick={() => {
                    const headers = ['Tên','Email','SĐT','Username','Link TikTok','Nền tảng','Lĩnh vực','Loại','Followers','Avg Views','Score','Tiềm năng','GMV kênh','Địa chỉ','GMV','Promo','Status','Ngày đăng ký'];
                    const gmvL = { under_1M:'< 1M', '1_10M':'1-10M', '10_50M':'10-50M', '50_100M':'50-100M', '100_300M':'100-300M', '300_1B':'300M-1B', over_1B:'> 1 tỷ' };
                    const rows = filtered.map(c => [
                      c.name, c.email, c.phone||'', getUN(c.tiktok_link), c.tiktok_link||'',
                      c.platform, c.niche, c.content_type,
                      c.followers, c.avg_views, scV(c), c.potential||'',
                      gmvL[c.channel_gmv]||'', c.address||'', c.gmv||0, c.promo_code||'', c.status,
              ...
              <div className="desktop-table" style={{ background: '#fff', border: '1px solid #f0f0f0', borderRadius: 18, overflow: 'hidden' }}>
                <div style={{ overflowX: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 840 }}>
                    <thead><tr>{['Creator','SĐT','Username','Link','Nền tảng','Lĩnh vực','Loại','Followers','Score','Tiềm năng','GMV kênh','Địa chỉ','GMV','Promo','Status'].map(h => <th key={h} style={TH}>{h}</th>)}</tr></thead>
                    <tbody>
                       ...
                       filtered.map(c => {
                        ...
                        return (
                          <tr key={c.id} style={{ cursor: 'default' }}>
                            ...
                            <td style={TD}>{c.phone ? <a href={`tel:${c.phone}`} style={{ fontSize: 12, fontWeight: 700, color: '#0f0f1a', textDecoration: 'none', background: '#f4f4f5', borderRadius: 8, padding: '3px 8px', whiteSpace: 'nowrap' }}>📞 {c.phone}</a> : <span style={{ color: '#a1a1aa', fontSize: 11 }}>—</span>}</td>
                            <td style={TD}>
